...
class Config {
  ...
  public getDatabaseConfig = (key: string) => {
    const databases = this.nConfig.get('databases');
    if (!databases || !databases[key]) {
      throw new Error(`Database configuration for key "${key}" not found`);
    }
