import { userMasterDao, categoryDao, blogDao, paragraphDao, paragraphImageDao } from '../../db/daos';
      ...
      for (let i = 0; i < paragraphs.length; i++) {
        ...
        if (paragraph.id) {
          ...
          if (existingParagraphsMap.has(paragraph.id)) {
            ...
            if (paragraph.image) {
              const existingImage = await paragraphImageDao.getParagraphImagesByParagraphId(paragraph.id);
